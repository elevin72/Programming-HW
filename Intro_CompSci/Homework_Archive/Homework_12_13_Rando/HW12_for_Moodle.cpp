...
void AddItem(Item* store, int maxItems, int& numItems){
    ...
    if(maxItems == numItems){ // if we have already inputted the max number of items then Error and break
        cout << "ERROR" << endl;
        return;
    }
...
void Print(Item* store, int numItems){
    for (int i = 0; i < numItems; i++){
        ...
        cout << "price: " << store[i].price << endl << endl; //floats loose accurracy, therefore when dealing with important decimal points, such as money, store as whole number and just cast  to float divide by 100 before printing
    }
}
...
void FindPrice(Item* store, int numItems){
    ...
    cin >> desiredCode;
    for (int i = 0; i < numItems; i++){
        if(desiredCode == store[i].code){   (store+i)->code
